...
### 3.2 防截断、跨格污染与低质量硬性规则

从 2026-06-13 起，所有新生成或刷新资源必须通过以下规则：

- 最终进入 `Content/` 的美术资源可以来自单资源母版，也可以来自安全 atlas；安全 atlas 必须逐项保存源图、逐项后处理、逐项验收，不能把整张 atlas 或贴边裁切结果直接作为最终资源。
- 图片模型母版必须为单个资源或安全 atlas。安全 atlas 中每两个素材的可见像素之间必须至少间隔 100px 纯色背景 gutter，外边缘也必须保留至少 80px 纯色背景。任何发光、符文、吊坠、卷轴轴头、武器尖端都不得进入 gutter。
- 批量 atlas 必须有明确网格隔离：生成提示中要要求“each sprite is centered in its own large cell, at least 100 pixels of pure #ff00ff empty gutter between neighboring sprites, no sprite enters another cell, no cropped top or bottom, no shared fragments”。如果模型输出无法保持隔离，必须改为更少格数或单资源生成。
- 切 atlas 时不得贴着素材或只相信固定格坐标。必须先按大 cell 裁出包含完整 gutter 的区域，再检查 cell 边缘安全带；若任一素材的非透明像素落入 cell 边缘安全带或 gutter，整张 atlas 不得进入最终资源，必须重新生成。
- 如果发现 A 的顶部/底部出现在 B 中，A 和 B 都视为污染链成员；不能只擦掉 B 的残片，因为 A 本体也可能已经缺失。污染链成员必须回到单资源 prompt 重新生成，并分别保存单独母版，直到每个资源都是完整独立图。
- 曾出现跨格污染的类型（Buff、物品图标、材料、饰品、召唤物、制作台 item icon）后续最终版只能使用单资源母版或满足 100px gutter 的安全 atlas；禁止使用紧密排布 atlas。
- 后处理允许裁掉母版色键背景，但导出到 `Content/` 的非平铺资源必须重新放回目标画布，并至少保留 1px 透明安全边；Buff、物品、弹幕、NPC/Boss、非平铺 object tile 和制作台 tile 都属于非平铺资源。
- “没有触边”不等于合格。1px 透明边在 Wiki、contact sheet、浏览器缩放或游戏 UI 混合时仍会产生视觉裁切；最终 `Content/` 贴图必须保留可视安全边：32x32 Buff/物品图标至少 3px，24x24 或更小图标至少 2px，制作台/object tile 至少 2px，普通弹幕至少 2px，极薄 16x4 类功能弹幕至少 1px。
- 16x16 地形块、墙、矿石等明确需要无缝平铺的资源可以触碰画布边缘，但必须在 spec 或脚本中标记为 tileable；它们不能用“透明安全边”破坏平铺。
- 极细长弹幕、光柱、雷线等不能机械等比缩放到目标尺寸。如果等比缩放会退化成点、短棒或单色线，必须做专门适配：横向弹幕保持横向长度，竖向光柱保持竖向高度，必要时手工像素化重绘。
- Contact sheet 是审阅图，不得把资源放满格子；预览缩放必须保留明显暗色边距，避免把正常透明边误判成截断。
- 禁止再提交“圆形底板 + 单一符号”“纯色圆/方块/横线”“几何 UI 按钮”“单色纸片”等占位式低质量素材，除非该文件在代码中明确是不可见碰撞/调试辅助贴图。
- 非平铺资源的有效非透明颜色数量应体现材质和光影层次；如果一个最终资源只有个位数颜色，必须人工说明其功能原因，或重新生成/重绘。
