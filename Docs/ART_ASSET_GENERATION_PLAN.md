...
- 每轮素材刷新后必须运行 `python Tools\clean_art_fragments.py`、`python Tools\enforce_art_padding.py` 和 `python Tools\verify_art_quality.py`；清理工具处理跨格污染，padding 工具按规则收缩非平铺资源并保留画布尺寸，审计工具把残片、可视安全边和低颜色占位风险作为阻断项。
