...
- 最终进入 `Content/` 的美术资源必须逐一生成、逐一保存母版、逐一后处理、逐一验收；禁止把多资源 atlas 直接作为最终资源来源。
- 图片模型母版必须为单个资源或规则网格 atlas；每个资源在母版格子内至少保留约 20% 空白边距，任何发光、符文、吊坠、卷轴轴头、武器尖端都不得触碰格子边界。
- 批量 atlas 必须有明确网格隔离：相邻资源之间必须有纯色背景 gutter，生成提示中要要求“每个 cell 只包含一个完整资源，cell 内资源不得越界，不得把邻格素材的一部分放入本格”。如果模型输出无法保持隔离，必须改为单资源生成或更少格数生成。
- 切 atlas 时不得只相信固定格坐标。每个 cell 切出后必须做连通域检查：主体之外靠近顶部/底部的小条、小点、残影视为跨格污染，必须清除或重新生成；不能把邻格顶部/底部残片当作合法粒子保留。
- 如果发现 A 的顶部/底部出现在 B 中，A 和 B 都视为污染链成员；不能只擦掉 B 的残片，因为 A 本体也可能已经缺失。污染链成员必须回到单资源 prompt 重新生成，并分别保存单独母版，直到每个资源都是完整独立图。
- 批量 atlas 只允许作为概念探索或低风险草稿；被用户指出过跨格污染的类型（Buff、物品图标、材料、饰品、召唤物、制作台 item icon）后续最终版必须使用单资源生成，禁止再从多格 atlas 直接切出最终 `Content/` 贴图。
