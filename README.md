...
- 已新增观劫盘：可随身查看灵压、天劫倒计时和抗劫建议。
- 已保护突破消耗品：境界不匹配时不会消耗关键突破材料，并会给出提示。
- 已为突破与 Boss 召唤失败提示接入中英文进度本地化。
...
- 已统一 Boss 召唤检查：Boss 已存在或境界不足时不消耗召唤物，并显示原因。
- 已为 Boss 召唤物 Tooltip 标注召唤目标和境界要求。
- 已为生成敌怪与 Boss 接入 Bestiary 文本，图鉴中可查看生态、掉落和试炼定位。
- 已为浅层灵脉的手写敌怪、灵脉蠕虫和幼节接入 Bestiary 文本。
- 已增强灵脉蠕虫 Boss 阶段：半血召唤幼节并发射灵气弹，低血量进入更快的暴走阶段。
- 已为 Boss 阶段提示接入中英文进度本地化。
- 已接入宗门声望：Boss 击败会提升声望，并影响部分高阶 NPC 商店。
- 已新增宗门玉简：玩家可使用它读取当前世界和自身修行对应的下一步目标。
- 宗门玉简会显示当前宗门声望，便于判断高阶商店门槛。
