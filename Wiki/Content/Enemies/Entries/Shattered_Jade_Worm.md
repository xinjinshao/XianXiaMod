

## 当前美术素材

<!-- ART_SECTION:entry-art:START -->

| 素材 | 名称 | ID | 类型 | 尺寸 |
| --- | --- | --- | --- | --- |
| <img src="../../../../Assets/Final/shattered_jade_worm/shattered_jade_worm__base__v01.png" alt="碎玉虫 base" width="72"> | 碎玉虫 | `shattered_jade_worm` | `base` | 48x24 |

<!-- ART_SECTION:entry-art:END -->

## 美术资源

- 主体：48x24。Prompt：Terraria pixel enemy, xianxia theme, transparent background.

# 碎玉虫
[返回敌怪总览](../Overview.md)

## 定位
- ID: shattered_jade_worm | 阶段: Pre-Boss | 生态: [浅层灵脉](../../Biomes/Entries/Shallow_Spirit_Veins.md)

## 生成条件
| 生态 | 生成概率 |
|------|----------|
| 浅层灵脉 | 20% |
## 数值
| HP | 伤害 | 防御 | 击退抗性 | AI |
|-----|------|------|----------|-----|
| 60 | 14 | 4 | 40% | Crawler + Burrow Dash |
## 行为
- 前120tick贴地爬行(移速2.5)。之后钻地冲刺(移速9,向上偏转2)。碎石粒子效果。
## 掉落
| 物品 | 概率 | 数量 |
|------|------|------|
| 下品灵石 | 50% | 1-2 |
| 灵气凝胶 | 33% | 1-2 |

## 代码实现

- ✅ 数值与wiki对齐
- ✅ 独特AI行为
- ✅ 掉落表
